...
The intended product scope includes actual pagination, server-side activity
filtering, and a visible revision-change state. These details need confirmation
as part of #4058. Exports, retention-policy changes, retroactive repricing, and
general-purpose query infrastructure are outside this design.

## Storage query interface

Proposed operation names below describe the contract, not finalized wire names:
...
Breakdown and pricing collections also have bounded first pages. A large number
of providers, models, tools, or overrides must not turn the initial response
into an unbounded array. Their continuation reads use the same revision check
and are requested by the user, not drained by a background loop. Exact summary
...
For an initial screen query, the existing Usage writer admits at most one repair
pass using its existing run/event limits, then commits it before starting the
read transaction. Repair is a write and must not be hidden inside a read-only
transaction. The screen read does not repeatedly repair until history is caught up.
...
Page continuation performs no new repair pass. Independently committed repair
or source changes invalidate the expected revision when they affect the screen.
...
The proposed default is a root-scoped durable Usage change counter read alongside
the existing pricing revision, bound on the wire to the Host generation and a
query identity. It is invalidation metadata, not a second collection of Usage
facts. The concrete representation and writer coverage require Storage review.

The counter must advance in the same committed transaction as any mutation that
changes a screen result, including:
...
An implementation must enumerate real writer paths, including cascade deletion
and repair, before choosing counter updates at write admission versus narrowly
scoped SQLite triggers. A root-wide counter is initially conservative: writes
...
The migration must preserve historical records. Some Usage rows intentionally
survive Session deletion and cannot be reconstructed by replaying surviving
...
Aggregate over the composed scalar relation before paginating groups. Taking
each source's top groups independently and merging them can omit the true top
...
Filters are applied in Storage before pagination. Current activity search is a
case-insensitive substring over model/provider/tool names plus a status filter.
...
page and 48 KiB per activity page, with bounded text fields. Group/pricing pages
and the complete screen envelope require separate explicit caps in the protocol;
...
as stale, and perform at most one automatic first-page reload for that user
action. Install the complete new screen atomically. Do not append new rows to
old statistics. A failed refresh retains an explicitly stale screen with manual
Refresh available; it does not enter an automatic retry loop. Continuous writes
may interrupt browsing, so that trade-off must be exercised under live activity.
...
Pagination of breakdowns and pricing is visible when required. Navigation memory
has a fixed bound and never grows with all visited activity rows.
...
or a fixed latency improvement without measurement. Aggregate scan and sort
work need a separately enforced execution budget and a typed budget outcome.
The implementation must demonstrate how that budget can interrupt or bound work
on the project's synchronous SQLite setup; a timer that fires only after a
blocking query returns is not enforcement.

If exact aggregation cannot meet an agreed interactive budget, review an
incrementally maintained aggregate projection or an explicitly limited outcome
before widening scope. Never present partial aggregate results as complete.
No latency threshold is claimed as an existing project guarantee.

## Verification contract

Storage tests exercise the public screen/page interface with real SQLite:

- concurrent append, correction, delete, pricing update, and repair cannot mix
  revisions; rollback and no-op behavior match the counter contract;
...
- extra group/pricing pages remain bounded and revision-checked;
- migrations preserve history after Session deletion and resume from a watermark.
...
high-cardinality breakdowns, sparse filters, and a pending-repair fixture. Record
