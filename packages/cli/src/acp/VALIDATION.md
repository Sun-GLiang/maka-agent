...
After PR #4862 merged, the branch was rebuilt as one PR5 commit on current Apache
main commit `6dfec43fe201607584fe148ead6093d22586b70e`.
...
| `node scripts/protocol-epoch-check.mjs --base upstream/main` | Passed: changed protocol, epoch 151 → 152. |
| `node --test scripts/protocol-epoch-check.test.mjs` | 17 passed. |
| `npm run lint` / `npm run format:check` | Passed (3570 linted files, 2115 formatted files). |
| `git diff --check upstream/main` | Passed. |
| MCP workspace tests | 250 passed. |
| Runtime Host workspace tests | 1930 passed, 12 skipped, including UDS scope isolation and existing Desktop/default registrations. |
| Core grant decoder test | Passed, including `mcp` and retained `desktop_mcp`. |
| CLI workspace tests | 1109 passed, 3 skipped, including the real ACP process boundary and all PR5 unit/integration suites. |
| Desktop and UI `knip` checks | Passed. |
| Desktop E2E | Side Chat follow-up acceptance passed 10/10 under an isolated stress loop and in the then-current full 35-test suite. After the final main rebase, the 36-test budget check passed, all five stress attempts completed the functional assertions, one timed out only while capturing the final screenshot, and an exact rerun passed. |
