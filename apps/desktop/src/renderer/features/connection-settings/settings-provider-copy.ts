...
const zhCapabilitiesCopy = {
  ...
  applyPatchHelp: '已知支持补丁编辑的模型默认启用，其余默认关闭。可为当前模型单独修改；关闭后使用 Write/Edit。',
...
const zhTwCapabilitiesCopy = {
  ...
  applyPatchHelp: '已知支援補丁編輯的模型預設啟用，其餘預設關閉。可為目前模型單獨修改；關閉後使用 Write/Edit。',
...
const enCapabilitiesCopy = {
  ...
  applyPatchHelp: 'Enabled by default for models known to support patch editing, disabled for other models. Change this setting for the current model; disabling uses Write/Edit.',
